                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex-1">
                                <div class="space-y-2 text-sm">
                                    ...
                                    <p><strong>Total Cost:</strong> ₱${parseFloat(
                                      billing.total_project_cost || 0
                                    ).toLocaleString()}</p>
                                    <p><strong>Balance:</strong> ₱${parseFloat(
                                      billing.remaining_balance || 0
                                    ).toLocaleString()}</p>
                                    <p><strong>Date:</strong> ${formatDate(
                                      billing.submission_date
                                    )}</p>
                                </div>
                            </div>
                            ...
                            ${
                              billing.receipt_image
                                ? `
<div class="ml-4">
    <img src="../../${billing.receipt_image}" 
         alt="Receipt" 
         class="w-20 h-20 object-cover rounded-lg cursor-pointer receipt-preview" 
         onclick="showReceiptModal('../../${billing.receipt_image}')">
</div>
                            `
                                : ""
                            }
...
function applyFilters() {
  ...
  let filtered = allBillings.filter((billing) => {
    ...
    const matchesDate =
      !dateFilter || billing.submission_date.startsWith(dateFilter);
...
async function markAsReceived(referenceNumber) {
  try {
    ...
    if (data.success) {
      showNotification(
        "Payment marked as received and notification sent!",
        "success"
      );
...
function showNotification(message, type = "info") {
  ...
  notification.className = `border-l-4 p-4 rounded shadow-lg min-w-80 ${
