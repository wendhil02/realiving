...
function createSizeCard(size) {
    ...
    sizeCard.innerHTML = `
        <div class="size-info">
            <h4>${escapeHtml(size.size_name)}</h4>
            ${size.dimensions ? `<p class="size-dimensions">${escapeHtml(size.dimensions)}</p>` : ''}
            ${parseFloat(size.price) > 0 ? 
                `<p class="size-price">+₱${parseFloat(size.price).toFixed(2)}</p>` : 
                '<p class="size-no-price">No extra cost</p>'}
        </div>
...
function createColorCard(color) {
    ...
    colorCard.innerHTML = `
        ${color.color_image ? `<img src="../../${escapeHtml(color.color_image)}" alt="${escapeHtml(color.color_name)}" class="color-preview" />` : ''}
        <div class="color-info">
            <div class="color-sample" style="background-color: ${escapeHtml(color.color_code || '#ccc')}"></div>
            <span>${escapeHtml(color.color_name)}</span>
        </div>
        ${parseFloat(color.price) > 0 ? `<p class="color-price">+₱${parseFloat(color.price).toFixed(2)}</p>` : ''}
        <div class="selection-indicator hidden">
            <i class="fas fa-check-circle"></i>
        </div>
    `;
