...
document.addEventListener('click', function(e) {
    // Remove type
    if (e.target.classList.contains('remove-type') || e.target.parentElement.classList.contains('remove-type')) {
        e.target.closest('.type-section').remove();
    }

    // Add size
    if (e.target.classList.contains('add-size') || e.target.parentElement.classList.contains('add-size')) {
        const btn = e.target.classList.contains('add-size') ? e.target : e.target.parentElement;
        const typeIdx = btn.getAttribute('data-type-index');
        const container = document.querySelector(`.sizes-container[data-type-index="${typeIdx}"]`);
    ...
    // Add color
    if (e.target.classList.contains('add-color') || e.target.parentElement.classList.contains('add-color')) {
        const btn = e.target.classList.contains('add-color') ? e.target : e.target.parentElement;
        const typeIdx = btn.getAttribute('data-type-index');
        const container = document.querySelector(`.colors-container[data-type-index="${typeIdx}"]`);
