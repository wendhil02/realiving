...
try {
    ...
    if ($receipt_image && $receipt_image['error'] === UPLOAD_ERR_OK) {
        ...
        // ✅ Insert into billing_submissions table (always insert new record)
        $stmt = $conn->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->bind_param("sssssss", $reference_number, $client_name, $total_project_cost, $remaining_balance, $fileName, $status, $received_date);
